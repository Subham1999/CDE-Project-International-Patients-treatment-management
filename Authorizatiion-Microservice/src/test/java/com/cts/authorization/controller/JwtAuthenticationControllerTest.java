...
class JwtAuthenticationControllerTest {
	...
	private User dummyAdminUser() {
		return new User(1, "admin", "pass", "ROLES_ADMIN");
	}
...
//		assertThat(controller.authorizeTheRequest("Bearer token")).
	}

	@Test
	public void testAuthorizationInvalid() throws Exception {
		User user = dummyAdminUser();// User(1,"admin", "pass");
		UserDetails details = new org.springframework.security.core.userdetails.User(user.getUserName(),
				user.getPassword(), new ArrayList<>());
		when(userDetailsService.loadUserByUsername("admin")).thenReturn(details);
